...
export default function Header() {
  ...
  const generatePdf = async (type: 'class' | 'teacher') => {
    ...
    const generatePage = (
        ...
        const { timeSlots, days } = templateTimetable;
        ...
        const formatClassName = (className: string) => {
            ...
            return `${gradePart}${armPart}`.replace(/ /g, '');
        };
    ...
    if (type === 'class' && activeTimetable) {
        ...
        allTeachers.forEach(teacher => {
            ...
            if (timetablesForTeacher.length > 0) {
                ...
                if (allTeacherSessions.length === 0) return;
                ...
                generatePage(`${teacher.name}'s Timetable`, teacher.id, 'teacher', [consolidatedTimetable], allTeacherSessions);
            }
        });
    } else {
        return;
